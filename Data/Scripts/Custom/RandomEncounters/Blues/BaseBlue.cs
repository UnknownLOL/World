...
{ 
	...
	{ 
		...
		{
            if ( m is BaseBlue || m is BaseVendor || m is BasePerson || m is Citizens || m is PlayerVendor || m is TownHerald ) //|| m is Townsperson )
				return false;
			...
			if ( m is BaseCreature )
			{
				...
				if( c.Controlled || c.Karma >= 0 || c.FightMode == FightMode.None )
					return false;
		...
		public override void OnMovement( Mobile m, Point3D oldLocation )
		{
			if ( !m.Frozen && DateTime.Now >= m_NextResurrect && InRange( m, 4 ) && !InRange( oldLocation, 4 ) && InLOS( m ) )
			{
				...
				else if ( this.HealsYoungPlayers && m.Hits < m.HitsMax && m is PlayerMobile && !m.Criminal)
				{
