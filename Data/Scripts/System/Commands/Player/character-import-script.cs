        ...
        private static void ImportCharacterData(Mobile from, PlayerMobile target, string fileName)
        {
            try
            {
				...
				foreach (XmlNode skillNode in skillNodes)
				{
					...
					{
						from.SendMessage(String.Format("Unrecognized skill: {skillName}", skillName));
					}
				}
                ...
                foreach (XmlNode itemNode in itemNodes)
                {
                    ...
                    if (type != null && type.IsSubclassOf(typeof(Item)))
                    {
                        ...
                        catch (MissingMethodException)
                        {
                            ...
                            from.SendMessage(String.Format("Unable to create item of type {itemType}. No parameterless constructor available.", itemType));
                        }
                    }
                    else
                    {
                        from.SendMessage(String.Format("Invalid item type: {itemType}", itemType));
                    }
                }
